...
window.onload = function(){
	...
	n0.value = 10;
	r.value = 2;
	tau.innerHTML = Math.trunc(1000*Math.log(2)/r.value)/1000;
}
function dispInfo(){
	window.alert("N0 est la population initiale, N0 > 0, choisir un nombre entre 0.01 et 2000\nr est le taux d’accroissement, r>0, choisir un nombre entre 0.002 et 69\ntau (ou T) est le temps de doublement et vaut tau = ln(2)/r");
}
class Graph{
	...
	drawFValues(f){
		...
		this.drawYVal(f.n0*2,f.style,"2N0");
		this.drawYVal(f.n0*4,f.style,"4N0");
	}
	resetGraph(){
...
class Aff{
	...
	draw(){
		...
		for(var i=0;i<5;i++){
			if(this.f[i]!=null){
				maxX = Math.max(maxX, 2.5*this.f[i].tau());
				maxY = Math.max(maxY, 5*this.f[i].n0);
			}
		}
...
function add(){
	if(r.value>=0.002&&r.value<=69&&n0.value>=0.01&&n0.value<=2000){
		aff.addF(new Exp(n0.value, r.value, "black"));
	}else{
		dispInfo();
	}
}
